...
| 소비자 | **script-agent·hub·infra** | profile 둔 모든 repo (opt-in) — 현재 **infra** | profile 둔 모든 repo (현재 **infra**, **meta 포함 가능**) |
...
- **infra도 활성 소비자**다(`enabledPlugins: harness@monitoring` + `codex-gate`·`write-guard`·
  `proposal-review` profile 보유 → Stop 게이트·PreToolUse 가드·proposal-review 실사용). 단 infra는
  `CLAUDE.md`·`.claude/agents`(analyzer/implementer) 파이프라인이 없는 **경량 소비자**다 —
  Stop 게이트 + PreToolUse 가드 + proposal-review profile만 두고, native codex-gate cutover는 해당 없음.
