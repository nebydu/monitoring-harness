...
**결정: NO-GO 확정 (A안 — meta 자체 gate 유지).** 이 플러그인은 **런타임(코드 실행) 하네스 전용**인데,
meta는 **코드를 실행하지 않고 spec 일관성을 판정**하는 다른 범주다. 게다가 meta의 gate(278줄)는
hub/script-agent(193/195줄)와 달리 **공통 골격의 상위집합(superset)** 으로 진화해 있어, 현재 core로
전환하면 **기능 회귀**가 발생한다. 따라서 meta는 *드리프트 관리에 실패한 예외*가 아니라 **애초에 이
런타임 하네스 플러그인의 적용 범주가 아닌** 산출물로 둔다(자체 gate 유지). meta까지 정식 통합이
**필요해질 때만** core를 meta 수준으로 확장하는 B안(core v2)을 검토한다(선택적 후속, H6 후보).
...
- meta는 **코드 미실행**·spec 일관성 도메인이라 build/test 명령이 없다(이건 주입점으로 처리 가능, 문제 아님).
...
선택지:
- **A) meta 자체 gate 유지 (확정)**: meta는 가장 진화한 gate를 그대로 둔다. 이 플러그인은 런타임 하네스
  (hub/script-agent)만 공통화한 상태로 마무리하고, meta는 **범주 외 산출물**로 명시한다(드리프트 예외가
  아님 — 코드 미실행·spec 판정이라 처음부터 대상이 아니다).
- **B) core v2로 확장 후 통합 (후속 과제, 강제 요인 발생 시)**: 공통 core에 gate_key 캐싱·2-모드 프롬프트
  (예: 여러 prompt 슬롯 + 트리거 분류 주입)·JSON 상태를 흡수해 meta 수준으로 끌어올린다. 그러면
  hub/script-agent도 캐싱 이득을 본다. 단 **hub/sa 동등성 재검증**과 core 대규모 변경이 필요하므로,
  meta까지 정식 통합해야 할 **구체적 필요가 생길 때만** 착수한다 → 별도 milestone(H6 후보).

> meta 도메인 룰은 어느 경우에도 consumer(meta `.claude/`)에 남는다. plugin은 도메인 결정을 하지 않는다.

## 5. 결정 (사람 확인 게이트)

**A안 확정**: meta는 자체 gate를 유지하고, 공통 plugin codex-gate로 전환하지 않는다. meta는 이
**런타임 하네스 플러그인의 범주 밖** 산출물이다(코드 미실행·spec 판정 도메인 + gate가 상위집합이라
공통화가 곧 회귀). 즉 drift 관리의 결함 있는 예외가 아니라 **범주 경계**로 둔다. B안(core v2 확장)은
meta 정식 통합이 필요해질 때만 착수하는 선택적 후속 과제로 보류한다. meta repo는 **미수정**.
