...
- **H2-B 검증·staging 완료, 라이브 cutover(재시작) 대기**:
  wiring 표준(H5: plugin 모델 + **convention 경로** profile, 상태 `${CLAUDE_PLUGIN_DATA}`)으로 적용.
  - consumer delta `script-agent/.claude/codex-gate.profile`(미추적), project scope 설치(enabledPlugins 기록).
  ...
  - **라이브 활성화는 미수행(재시작 필요)**: 사용자 세션에서 `/plugin update` → 재시작·검증 → native Stop
    hook 비활성(중복 제거) → script-agent settings.json+profile 커밋. 현재 `.claude/hooks/codex-gate.sh`가
...
- **미수행(라이브 = H2-B cutover, 재시작 필요)**: Windows shim 경유 end-to-end Stop hook 발화,
  `codex exec` 실연동, native Stop hook 비활성. (profile 주입은 H2-B에서 convention으로 확정 — userConfig 불요)
