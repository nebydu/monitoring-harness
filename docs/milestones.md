...
- **H2-B 보류(사람 결정)**: script-agent 실제 적용은 **wiring 표준이 정해진 뒤로 보류**했다.
  현재 script-agent `.claude/`는 **무변경**.
  → **wiring 표준은 H5에서 확정됨**: plugin 모델 + consumer profile 주입(consumer는 profile 델타만,
  골격/스키마/배선은 plugin, 상태는 `${CLAUDE_PLUGIN_DATA}`). 상세 [`installation.md`](installation.md) §0.
  H2-B는 이 표준으로 script-agent 1개에 설치·구성 후 런타임 동등성을 사람 확인 게이트에서 검증한다(미수행).
