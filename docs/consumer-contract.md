...
> 현재 `plugin.json`은 `version: 0.1.0`을 고정하고 있어, 그대로 두면 hook 변경이 bump 없이는 전파되지
> 않는다. H2~H4 prototype 동안에는 commit-SHA 방식(또는 매 변경 시 version bump 규율)을 선택해야 한다.
> 어느 방식을 표준으로 삼을지는 H5 packaging에서 확정한다(→ [`milestones.md`](milestones.md) H5).
