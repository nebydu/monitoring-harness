...
## 2. consumer 구성 (profile + plugin config)

consumer는 **델타 profile 한 파일**만 둔다. 골격/스키마/배선은 plugin에서 온다.

1. profile 작성 — 템플릿은 [`../shared/hooks/profiles/script-agent.profile.example`](../shared/hooks/profiles/script-agent.profile.example).
   consumer repo의 `.claude/codex-gate.profile`로 복사 후 도메인 값(트리거/스킵/프롬프트)을 채운다.
2. plugin config에 profile 경로 주입 — Stop hook은 `hooks.json`에서 `${user_config.profile}`을
   profile 경로로 받는다. 예:

   ```
   profile = ${CLAUDE_PROJECT_DIR}/.claude/codex-gate.profile
   ```

   (`/plugin` 구성 UI 또는 settings의 plugin user config로 설정. 정확한 선언 키는 실제 설치 검증 시 확정 —
   아래 "미검증" 참고.)
...
## 6. 정적 검증 (H5에서 수행) / 미검증 (실제 설치 시)

- **수행**: `plugin.json`·`marketplace.json`·`hooks/hooks.json` JSON parse, `codex-gate-entry.sh`
  `bash -n`, (가능 시) `claude plugin validate`.
- **미검증(실제 consumer 설치 필요 → H2-B 게이트)**: Windows Git Bash shim 경유 end-to-end Stop hook
  실행, `${user_config.profile}` 주입 선언 키 확정, `codex exec` 실연동, settings.json 영향.
  이 검증은 첫 적용 대상 script-agent에서 사람 확인 게이트로 수행한다.
