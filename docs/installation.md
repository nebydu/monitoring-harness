...
> [`decisions/h4-meta-readiness.md`](decisions/h4-meta-readiness.md)). 아래 절차는 sa/hub가 따른 적용 표준이며, 신규 consumer가
> **런타임 하네스라면** 동일하게 쓴다.
...
| 보존 상태(escalation 카운터·log) | runtime | `${CLAUDE_PLUGIN_DATA}` (업데이트를 넘어 유지, `harness@monitoring` 기준 data key는 `harness-monitoring`) |
...
# 플러그인 설치 (project 스코프로 넣으면 repo 설정에 enabledPlugins가 기록됨)
...
- `--scope project`는 consumer `.claude/settings.json`의 `enabledPlugins`에 `harness@monitoring`을
  기록해 repo 설정으로 공유한다. 단 새 환경에서는 marketplace 등록과 plugin 설치/cache 생성이 먼저
  필요하다. 개인 시험용은 스코프 생략(user 스코프).
...
- profile 작성 — 템플릿은 [`../shared/hooks/profiles/script-agent.profile.example`](../shared/hooks/profiles/script-agent.profile.example).
  ...
  **repo에 커밋**한다(협업자 공유).
- 설치/cache 생성 이후에는 per-user 설정·절대경로 없이 project scope 설정만으로 동작한다.
