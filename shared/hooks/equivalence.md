...
> H2-A(harness 내부 산출물) 기준이다. 실제 script-agent 적용 후의 런타임 동등성 확인은 H2-B
> 시범 적용 + 사람 확인 게이트에서 수행한다(아래 "미수행" 참고).
...
## 런타임 동등성 (H2-B에서 수행 — 아직 미수행)

다음은 **consumer 수정이 필요**하므로 H2-B 시범 적용 + 사람 확인 게이트에서 한다:

- script-agent에서 core+profile wiring으로 교체 후, 동일 변경셋에 대해 trigger/skip 판정과
  pass/fail/escalation 동작이 기존과 일치하는지 실측.
- wiring 방식(vendor / relative / plugin user_config) 확정 및 settings.json 영향 확인.
