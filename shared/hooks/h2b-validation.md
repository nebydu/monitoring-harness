...
> 주: 아래 plugin id는 이후 **`harness@monitoring`** 으로 개명됐다(marketplace `monitoring` + plugin
> `harness`). 최초 cutover 시점에는 `monitoring-harness@monitoring-harness`였다. 명령은 현행 id로 갱신해 둔다.
...
claude plugin install harness@monitoring --scope project   # --config 불필요(convention)
# 2) convention 적용 골격 반영
claude plugin update harness@monitoring
...
- `claude plugin uninstall harness@monitoring` (또는 `enabledPlugins`에서 비활성)
